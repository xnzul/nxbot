/*
]=====> NzuXsex403 <=====[ ]=====> YT NzuXploit <=====[ ]=====> 083173142772 <=====[
*/
const {
   WAConnection,
   MessageType,
...
*/
const ownerNumber = ["6283173142772@s.whatsapp.net","6283173142772@s.whatsapp.net"]
const botName = '𝐍𝐗 𝐁𝐎𝐓'
const ownerName = '𝗡𝘇𝘂𝗹𝗫𝘀𝗲𝘅𝟰𝟬𝟰'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT NzuXploit'))
})

	client.on('credentials-updated', () => {
		fs.writeFileSync('./nzulxsex.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./nzulxsex.json') && client.loadAuthInfo('./nzulxsex.json')
	client.on('connecting', () => {
		start('2', 'nzulxsex Connecting...')
	})
	client.on('open', () => {
		success('2', 'nzulxsex Connected')
	})
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana bep? Contoh : ${prefix}nulis1 nzulxsex baik hati`)
				nzulxsex = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${nzulxsex}&APIKEY=${XteamKey}`)
				client.sendMessage(from, buff, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya bep :*'})
				await limitAdd(sender)
				break
				case 'nulis2':
				case 'tulis2':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana bep? Contoh : ${prefix}nulis2 nzulxsex baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸nzulxsex⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo nzulxsex & Gans`)
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub nzulxsex & Hub`)
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] Contoh : ${prefix}gemboktext 11 01 2021 & nzulxsex dan Nadia`)
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] Contoh : ${prefix}glitchtext nzulxsex & Gans`)
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Kode bahasanya mana bep | contoh : ${prefix}tts id ah yamate kudasai', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, `Teksnya mana bep | contoh : ${prefix}tts id ah yamate kudasai`, text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Teks nya terlalu panjang bep')
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isQuotedSticker) return reply('Reply atau Tag sticker yang mau dijadiin gambar bep >_<')
					reply(ind.wait())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih bep [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata nzulxsex`
					if (args.length < 1) return reply(pref)
					reply(ind.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=random`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Nih bep >_<', quoted: mek})
					await limitAdd(sender)
					break
                   case 'stalkig':
                   if (isBanned) return reply(ind.baned())
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                     ...
                     reply('「❗」Sabar Lagi Stalking IG nya bep')
                ...
                if (namaUser.length >= 30) return reply(`Namanya kepanjangan bep :)`)
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*FITUR WELCOME SUDAH AKTIF BEP*')
						welkom.push(from)
                 ...
                 case 'event':
                  if (isBanned) return reply(ind.baned())                 
					...
					if (args.length < 1) return reply('Ekhemm >_<')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*FITUR EVENT SUDAH AKTIF BEP*')
						event.push(from)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					...
					if (args[0].startsWith('08')) return reply('Gunakan kode bahasa bep')
					try {
           ...
           case 'demote':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					if (!isGroup) return reply(ind.groupo())                 
				if (args.length < 1) return reply(`Gini bep : ${prefix}fitnah [@tag&pesan&balasanbot]\n\nContoh : ${prefix}fitnah @tagmember&hai&hai juga`)
				var gh = body.slice(8)
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					break
                      ...
                      if (!isRegistered) return reply(ind.noregis())
                      if (isBanned) return reply(ind.baned())   
                      client.sendMessage(from, 'Aku pamit bep:)', text)
                      }, 0)
                      break																									
/*
]=====> DOWNLOAD MENU <=====[
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('URL NYA TIDAK VALID BEP')				
...
*/
                ...
                addKoinUser('6283173142772@s.whatsapp.net', fee)
				...
				if ( checkATMuser(sender) <= total) return reply(`maaf bep uang nya gak cukup, kumpulin uang nya dumlu >_< jangan open bo bep:v`)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner nzulxsex`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena nzulxsex baik Bot memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'bitly':
               ...
               client.updatePresence(from, Presence.composing) 
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                if (isGroup) return  reply( 'TIDAK BISA DI GRUP BEP')
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext nzulxsex`)
cloud = body.slice(11)
